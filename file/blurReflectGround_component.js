...
export default {
    ...
    id: 'blurReflectGround',

    name: '磨砂反射地面',

    create(storage, { scene, renderer, camera }) {

        const reflectorProps = storage?.reflectorProps ?? {}

        const material = new MeshReflectorMaterial(renderer, camera, scene, {}, {
            resolution: 1024,
            blur: [512, 128],
            mixBlur: reflectorProps.mixBlur ?? 5,
            mixStrength: reflectorProps.mixStrength ?? 5,
            mixContrast: reflectorProps.mixContrast ?? 1,
            mirror: reflectorProps.mirror ?? 1,
            parameters: {
                roughnessMap: new THREE.TextureLoader().load(HOST + 'images/drei/roughness.jpg'),
                normalMap: new THREE.TextureLoader().load(HOST + 'images/drei/normal.jpg'),
        ...
        const { reflectorProps: props } = material

        const pane = new Pane()

        pane.addBinding(props, 'mixBlur', { min: 0, max: 20 })

        pane.addBinding(props, 'mixStrength', { min: 0, max: 20 })

        pane.addBinding(props, 'mirror', { min: 0, max: 1 })

        pane.addBinding(props, 'mixContrast', { min: 0, max: 5 })

        const update = () => material.update()

        scene.addLoopRender(update)

        plane.ON_REMOVE_CALL = () => {

            scene.removeLoopRender(update)

            pane.dispose()

            material.fbo1.dispose()

            material.fbo2.dispose()

            material.kawaseBlurPass.dispose()
    ...
    setPanel(mesh, folder) {

        const { reflectorProps } = mesh.material

        folder.add(reflectorProps, 'mixBlur', 0, 20).name('模糊混合')

        folder.add(reflectorProps, 'mixStrength', 0, 20).name('反射强度')

        folder.add(reflectorProps, 'mirror', 0, 1).name('镜面')

        folder.add(reflectorProps, 'mixContrast', 0, 5).name('对比度')

    },

    getStorage(mesh) {

        const { mixBlur, mixStrength, mixContrast, mirror } = mesh.material.reflectorProps

        return { reflectorProps: { mixBlur, mixStrength, mixContrast, mirror } }
